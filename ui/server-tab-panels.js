                            ...
                            <div v-for="(inbound, idx) in serverInbounds" :key="inbound.id" class="bg-gray-50 p-4 rounded-xl border border-gray-200" @focusin.capture="queueJsonScrollTo('inbound', inbound)" @input.capture="queueJsonScrollTo('inbound', inbound)" @change.capture="queueJsonScrollTo('inbound', inbound)">
                                ...
                                <div v-if="!inbound.collapsed" class="space-y-4">
                                    ...
                                    <div v-if="inbound.type==='hysteria2'" class="bg-white border border-gray-200 rounded-lg p-3 grid grid-cols-2 gap-3">
                                        <div v-if="inboundHysteria2BandwidthFieldsVisible(inbound)"><label class="block text-[10px] font-black text-gray-400 uppercase mb-1 tracking-wider">上行带宽 (Up Mbps)</label><input v-model="inbound.hy2_up_mbps" class="w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-lg text-sm outline-none font-mono"></div>
                                        <div v-if="inboundHysteria2BandwidthFieldsVisible(inbound)"><label class="block text-[10px] font-black text-gray-400 uppercase mb-1 tracking-wider">下行带宽 (Down Mbps)</label><input v-model="inbound.hy2_down_mbps" class="w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-lg text-sm outline-none font-mono"></div>
                                        ...
                                        <div class="flex items-end"><label class="flex items-center gap-2 cursor-pointer"><input type="checkbox" v-model="inbound.hy2_ignore_client_bandwidth" @change="syncHysteria2BandwidthMode(inbound)" class="w-4 h-4 text-indigo-600 rounded"><span class="text-xs font-bold text-gray-700">忽略客户端带宽 (Ignore Client Bandwidth)</span></label></div>
                                    ...
                                    <div v-if="inbound.type==='shadowtls'" class="bg-white border border-gray-200 rounded-lg p-3 grid grid-cols-2 gap-3">
                                        ...
                                        <div v-if="inboundShadowTlsFieldVisible(inbound, 'shadowtls_password')"><label class="block text-[10px] font-black text-gray-400 uppercase mb-1 tracking-wider">密码 (Password)</label><input v-model="inbound.shadowtls_password" class="w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-lg text-sm outline-none font-mono"></div>
                                        ...
                                        <div v-if="inboundShadowTlsFieldVisible(inbound, 'shadowtls_handshake_for_server_name_text')" class="col-span-2"><label class="block text-[10px] font-black text-gray-400 uppercase mb-1 tracking-wider">按域名握手 (Handshake For Server Name)</label><textarea v-model="inbound.shadowtls_handshake_for_server_name_text" rows="3" placeholder="example.com=127.0.0.1:443&#10;api.example.com=127.0.0.1:8443" class="w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-lg text-xs outline-none font-mono resize-none"></textarea></div>
                                        <div v-if="inboundShadowTlsFieldVisible(inbound, 'shadowtls_strict_mode')" class="flex items-end"><label class="flex items-center gap-2 cursor-pointer"><input type="checkbox" v-model="inbound.shadowtls_strict_mode" class="w-4 h-4 text-indigo-600 rounded"><span class="text-xs font-bold text-gray-700">严格模式 (Strict Mode)</span></label></div>
                                        <div v-if="inboundShadowTlsFieldVisible(inbound, 'shadowtls_wildcard_sni')"><label class="block text-[10px] font-black text-gray-400 uppercase mb-1 tracking-wider">通配 SNI (Wildcard SNI)</label><select v-model="inbound.shadowtls_wildcard_sni" class="w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-lg text-sm outline-none"><option value="off">off</option><option value="authed">authed</option><option value="all">all</option></select></div>
                                    </div>
                                    ...
                                    <div v-if="inboundSupportsTls(inbound.type)" class="bg-white border border-gray-200 rounded-lg p-3 space-y-3">
                                        <div class="flex items-center justify-between">
                                            ...
                                            <label class="flex items-center gap-2 cursor-pointer"><input type="checkbox" v-model="inbound.tls_enabled" :disabled="inboundTlsRequired(inbound)" @change="syncInboundTlsState(inbound)" class="w-4 h-4 text-indigo-600 rounded disabled:opacity-60 disabled:cursor-not-allowed"><span class="text-sm font-bold text-gray-700">启用 TLS<span v-if="inboundTlsRequired(inbound)" class="text-[11px] text-gray-500 ml-1">(必需)</span></span></label>
                                        </div>
                                        <div v-if="inbound.tls_enabled" class="grid grid-cols-2 gap-3">
                                            ...
                                            <div v-if="inboundRealitySupported(inbound)" class="flex items-end"><label class="flex items-center gap-2 cursor-pointer"><input type="checkbox" v-model="inbound.reality_enabled" @change="syncInboundRealityState(inbound)" class="w-4 h-4 text-indigo-600 rounded"><span class="text-xs font-bold text-gray-700">启用 Reality</span></label></div>
                                            <template v-if="inboundRealityVisible(inbound) && inbound.reality_enabled">
                                    ...
                                    <div v-if="inboundSupportsTransport(inbound.type)" class="bg-white border border-gray-200 rounded-lg p-3 space-y-3">
                                        ...
                                        <div class="grid grid-cols-2 gap-3">
                                            <div><label class="block text-[10px] font-black text-gray-400 uppercase mb-1 tracking-wider">传输层 (Transport)</label><select v-model="inbound.transport" @change="syncInboundTransport(inbound)" class="w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-lg text-sm outline-none"><option value="">none (default)</option><option v-for="item in inboundTransportOptions(inbound)" :key="item" :value="item">{{ item }}</option></select></div>
